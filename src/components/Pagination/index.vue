  ...
  computed: {
    ...
    // 计算连续页码数
      ...
      const {
        currentPage,
        totalPages,
        pageConfig: { showPageNo }   // 从pageConfig解构出showPageNo
      } = this
      let start = 0 // 开始的页码
      let end = 0 // 结束的页码
      // 计算开始的页码
      ...
      // 根据判断,限定结束页码的值
      // 总页码:20  当前选中的页码:19 此时能够计算出结束页码是21,但是根本就没有21页
      if (end > totalPages) {
        end = totalPages
        // 修正的是开始的页码  20-5+1  多显示个16
        start = end - showPageNo + 1
        // 重新的再次修正开始页码  如果只有4页的情况
        if (start < 1) {
          start = 1
        }
      }
